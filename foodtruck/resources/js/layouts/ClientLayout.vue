...
<template>
    <div class="flex min-h-screen flex-col bg-amber-50 dark:bg-gray-950">
        <!-- Navbar -->
        <header class="sticky top-0 z-50 border-b border-amber-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <nav class="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
                ...
                <!-- Nav links -->
                <div class="hidden items-center gap-6 text-sm font-medium text-gray-700 dark:text-gray-200 sm:flex">
                    <Link href="/" class="transition hover:text-amber-600">{{ t('nav.home') }}</Link>
                    <Link href="/menu" class="transition hover:text-amber-600">{{ t('nav.menu') }}</Link>
                    <Link href="/#location" class="transition hover:text-amber-600">{{ t('nav.location') }}</Link>
                    <a href="#contact" class="transition hover:text-amber-600">{{ t('nav.contact') }}</a>
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <!-- Cart -->
                    <Link href="/cart" class="relative rounded-full p-2 transition hover:bg-amber-100 dark:hover:bg-gray-800">
                        <ShoppingCart class="h-5 w-5 text-amber-600" />
                        <span
                            v-if="count > 0"
                            class="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-amber-500 text-xs text-white"
                        >{{ count }}</span>
                    </Link>
        ...
        <!-- Footer -->
        <footer class="border-t border-amber-200 bg-white dark:border-gray-800 dark:bg-gray-900">
            <div class="mx-auto max-w-6xl px-4 py-12">
                <div class="grid gap-10 sm:grid-cols-3">

                    <!-- Col 1: Brand + redes sociales -->
                    <div>
                        <p class="mb-1 text-xl font-bold text-amber-600">🚚 FoodTruck</p>
                        <p class="mb-5 text-sm text-gray-500 dark:text-gray-400">{{ t('footer.tagline') }}</p>
                        <p class="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-400">{{ t('footer.followTitle') }}</p>
                    ...
                    <div>
                        <p class="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-400">{{ t('footer.legalTitle') }}</p>
                        <ul class="space-y-2 text-sm">
                            <li>
                                <Link href="/legal/privacidad" class="text-gray-500 transition hover:text-amber-600 dark:text-gray-400 dark:hover:text-amber-400">
                                    {{ t('footer.privacy') }}
                                </Link>
                            </li>
                            <li>
                                <Link href="/legal/terminos" class="text-gray-500 transition hover:text-amber-600 dark:text-gray-400 dark:hover:text-amber-400">
                                    {{ t('footer.terms') }}
                                </Link>
                            </li>
                            <li>
                                <Link href="/legal/cookies" class="text-gray-500 transition hover:text-amber-600 dark:text-gray-400 dark:hover:text-amber-400">
                                    {{ t('footer.cookies') }}
                                </Link>
                            </li>
                        </ul>
            ...
            <!-- Bottom bar -->
            <div class="border-t border-amber-200 py-4 text-center text-xs text-gray-400 dark:border-gray-800">
