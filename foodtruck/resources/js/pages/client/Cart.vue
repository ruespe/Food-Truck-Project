...
<template>
    ...
    <div class="mx-auto max-w-2xl px-4 py-10">
        <h1 class="mb-6 text-3xl font-bold text-gray-800 dark:text-white">{{ t('cart.title') }}</h1>

        <div v-if="items.length === 0" class="rounded-2xl bg-white p-10 text-center text-gray-500 shadow dark:bg-gray-800 dark:text-gray-400">
            {{ t('cart.empty') }} <a href="/menu" class="text-amber-500 underline">{{ t('cart.seeMenu') }}</a>
        </div>
        ...
        <div v-else>
            <ul class="mb-6 divide-y rounded-2xl bg-white shadow dark:divide-gray-700 dark:bg-gray-800">
                <li v-for="item in items" :key="item.id" class="flex items-center gap-4 p-4">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-amber-600">{{ parseFloat(String(item.price)).toFixed(2) }} {{ t('cart.unit') }}</p>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <span class="w-6 text-center text-sm font-semibold dark:text-white">{{ item.quantity }}</span>
                        <button class="rounded-full bg-gray-100 px-2 py-1 text-sm font-bold hover:bg-gray-200 dark:bg-gray-700 dark:text-white dark:hover:bg-gray-600" @click="updateQty(item.id, item.quantity + 1)">+</button>
                    </div>
            ...
            <div class="flex items-center justify-between rounded-2xl bg-white p-5 shadow dark:bg-gray-800">
                <div>
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('cart.total') }}</p>
