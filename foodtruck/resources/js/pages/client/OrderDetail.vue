...
const props = defineProps<{
    order: {
        ...
        items: Array<{
            ...
            quantity: number;
            price: number;
            product: { id: number; name: Record<string, string>; image: string | null; price: number };
        }>;
...
function reorder() {
    props.order.items.forEach((item) => {
        add(
            { id: item.product.id, name: item.product.name, price: item.price, image: item.product.image },
            item.quantity,
        );
    });
    router.visit('/cart');
...
<template>
    ...
    <div class="mx-auto max-w-2xl px-4 py-10">
        ...
        <Link href="/orders" class="no-print mb-6 inline-flex items-center gap-2 text-sm text-gray-500 transition hover:text-amber-500 dark:text-gray-400">
        ...
        <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
            <div>
                <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white">
                    {{ t('orderDetail.title') }}<span class="font-mono text-orange-500">{{ order.ref }}</span>
                </h1>
        ...
        <!-- Payment info -->
        <div class="mt-4 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100 dark:bg-gray-800 dark:ring-gray-700">
            <div class="border-b border-gray-100 px-6 py-4 dark:border-gray-700">
            ...
            <div class="flex flex-wrap items-center justify-between gap-2 px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                <div class="space-y-1">
                    <p class="flex items-center gap-2">
                        <span class="text-gray-400">{{ t('orderDetail.paymentMethod') }}:</span>
                        <span class="font-medium">Stripe</span>
                    </p>
                    <p v-if="order.payment?.transaction_id" class="font-mono text-xs text-gray-400">
