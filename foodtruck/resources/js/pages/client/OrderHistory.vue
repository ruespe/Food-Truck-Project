...
<template>
    ...
    <div class="mx-auto max-w-3xl px-4 py-10">
        <h1 class="mb-6 text-3xl font-bold text-gray-800 dark:text-white">{{ t('orders.title') }}</h1>

        <div v-if="orders.length === 0" class="rounded-2xl bg-white p-10 text-center text-gray-500 shadow dark:bg-gray-800 dark:text-gray-400">
            {{ t('orders.empty') }} <a href="/menu" class="text-amber-500 underline">{{ t('orders.seeMenu') }}</a>
        </div>
            ...
            <li v-for="order in orders" :key="order.id">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="font-semibold text-gray-800 dark:text-white">{{ order.created_at }}</p>
                    </div>
