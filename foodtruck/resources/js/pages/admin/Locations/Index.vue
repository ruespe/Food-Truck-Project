...
<template>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Ubicación del truck</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Programa dónde estará el food truck cada día</p>
        </div>
    ...
    <div v-if="showForm" class="mb-6 grid gap-6 rounded-2xl bg-white border border-slate-200 p-6 shadow-sm dark:bg-slate-800 dark:border-[#66c0f4]/50 lg:grid-cols-2">
        <form class="space-y-3" @submit.prevent="submit">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="text-xs text-slate-500 dark:text-slate-400">Latitud</label>
                    <input :value="parseFloat(String(form.latitude)).toFixed(7)" type="text" readonly class="w-full rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-sm text-slate-600 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-300" />
                </div>
                <div>
                    <label class="text-xs text-slate-500 dark:text-slate-400">Longitud</label>
                    <input :value="parseFloat(String(form.longitude)).toFixed(7)" type="text" readonly class="w-full rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-sm text-slate-600 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-300" />
                </div>
            </div>
            <input v-model="form.date" type="date" required class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 focus:border-amber-500 focus:outline-none dark:border-slate-600 dark:bg-slate-700 dark:text-white" />
            <div class="grid grid-cols-2 gap-3">
                <input v-model="form.start_time" type="time" required class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 focus:border-amber-500 focus:outline-none dark:border-slate-600 dark:bg-slate-700 dark:text-white" />
                <input v-model="form.end_time" type="time" required class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 focus:border-amber-500 focus:outline-none dark:border-slate-600 dark:bg-slate-700 dark:text-white" />
            </div>
            <p class="text-xs text-slate-500 dark:text-slate-500">Arrastra el pin en el mapa para ajustar la ubicación exacta.</p>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-200 dark:border-[#66c0f4]">
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">Coordenadas</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    <tr v-for="loc in locations" :key="loc.id" class="transition hover:bg-slate-50 dark:hover:bg-slate-700/30">
                        <td class="px-6 py-4 font-medium text-slate-900 dark:text-white">{{ loc.name }}</td>
                        <td class="px-6 py-4 text-slate-600 dark:text-slate-300">{{ loc.date.split('-').reverse().join('/') }}</td>
                        <td class="px-6 py-4 text-slate-600 dark:text-slate-300">{{ loc.start_time.slice(0,5) }} – {{ loc.end_time.slice(0,5) }}</td>
                        <td class="px-6 py-4 font-mono text-xs text-slate-500">{{ loc.latitude }}, {{ loc.longitude }}</td>
                        <td class="px-6 py-4">
                            <div class="flex gap-2">
                                <button class="rounded-lg border border-slate-200 bg-white px-3 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50 transition dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600" @click="openEdit(loc)">Editar</button>
