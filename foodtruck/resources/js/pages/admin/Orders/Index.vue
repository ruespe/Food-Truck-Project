...
const props = defineProps<{
    orders: Paginator;
    filters: { status?: string; client?: string; date_from?: string; date_to?: string };
    statuses: string[];
}>();
...
const filterClient   = ref(props.filters.client   ?? '');
...
watch([filterClient, filterDateFrom, filterDateTo], debouncedApply);
...
const hasFilters = () =>
    filterStatus.value || filterClient.value || filterDateFrom.value || filterDateTo.value;
...
<template>
    ...
    <!-- Filters -->
    <div class="mb-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-[#66c0f4]/50 dark:bg-slate-800">
        <div class="flex flex-wrap gap-3">
            ...
            <button
                v-if="hasFilters()"
                @click="clearFilters"
                class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-500 transition hover:bg-red-50 hover:text-red-500 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-400"
            >
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-200 dark:border-[#66c0f4]">
                        <th class="px-6 py-3 text-left text-xs font-medium tracking-wide text-slate-500 uppercase dark:text-slate-400">#</th>
                        <th class="px-6 py-3 text-left text-xs font-medium tracking-wide text-slate-500 uppercase dark:text-slate-400">Cliente</th>
                        <th class="px-6 py-3 text-left text-xs font-medium tracking-wide text-slate-500 uppercase dark:text-slate-400">Productos</th>
                        <th class="px-6 py-3 text-left text-xs font-medium tracking-wide text-slate-500 uppercase dark:text-slate-400">Total</th>
                        <th class="px-6 py-3 text-left text-xs font-medium tracking-wide text-slate-500 uppercase dark:text-slate-400">Estado</th>
                        <th class="px-6 py-3 text-left text-xs font-medium tracking-wide text-slate-500 uppercase dark:text-slate-400">Fecha</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                    ...
                    >
                        <td class="px-6 py-4 font-mono text-xs text-amber-500 dark:text-amber-400">
                            {{ order.ref }}
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="max-w-xs truncate px-6 py-4 text-slate-500 dark:text-slate-400">
                            <span v-for="(item, i) in order.items" :key="i">
                                {{ item.quantity }}× {{ td(item.product.name)
                                }}<span v-if="i < order.items.length - 1">, </span>
                            </span>
                        </td>
                        <td class="px-6 py-4 font-bold text-amber-600 dark:text-amber-400">
                        ...
                        <td class="px-6 py-4">
                            ...
                            >
                                <option v-for="s in statuses" :key="s" :value="s" class="bg-slate-800 text-white">
                                    {{ statusLabel(s) }}
                                </option>
                            </select>
                        </td>
